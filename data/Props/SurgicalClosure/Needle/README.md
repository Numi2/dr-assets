...
- `Props/SurgicalClosure/Needle/ExperimentalSurfaceFEM/`
  - retired ribbon-like surface-FEM thread and violet v0.3 adapter experiments;
  - preserved only for reproducibility and deliberately outside the selectable
    puncture/pullout catalog;
  - rejected after real RTX renders exposed excessive tissue deformation.
...
The segmented assemblies are raw USD maximal-coordinate physics. They are not
exposed as Isaac Lab reduced-coordinate articulations because their threads
use general D6 joints. The active puncture/pullout adapter is maintained in the
parent DrAnmar repository and uses the authored braided-suture asset family,
not the retired surface-FEM experiment.
...
All representations are simulator-engineering evidence only, not
biomechanical or clinical validation of a particular suture.
